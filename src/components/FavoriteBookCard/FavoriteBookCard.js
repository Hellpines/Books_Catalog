...
export const createFavoriteBookCard = (book, onToggleFavorite) => {
    const card = document.createElement('div');
    card.className = 'favoriteBookCard';

    const coverHtml = book.cover_i
        ? `<img src="https://covers.openlibrary.org/b/id/${book.cover_i}.jpg" alt="${book.title}" class="favCover" />`
        : `<div class="favNoCover">No cover</div>`;

    card.innerHTML = `
        ${coverHtml}
        <div class="favInfo">
            <h4 class="favTitle">${book.title}</h4>
            <p class="favAuthor">${book.author_name ? book.author_name.join(', ') : 'Unknown Author'}</p>
            <p class="favYear">${book.first_publish_year || 'Unknown Year'}</p>
        </div>
        <button class="favButton">
            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                ...
                fill="var(--red)" stroke="var(--red)" stroke-width="1.33333" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
        </button>
    `;
